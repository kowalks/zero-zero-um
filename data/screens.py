...
class Screen:
    ...
    def __init__(self, caption="My game name", *args, **kwargs):
        # Initialize screen
        self.scn = pygame.display.set_mode((settings.SCREEN_WIDTH, settings.SCREEN_HEIGHT))
...
class TitleScreen(Screen):
    ...
    def run_events(self, running):
        ...
        ng_button = buttons.Button(settings.MARGIN, settings.SCREEN_HEIGHT - settings.MARGIN, "Novo jogo",
                                   "blue_button")
        controls_button = buttons.Button(settings.MARGIN + settings.BT_DIST, settings.SCREEN_HEIGHT - settings.MARGIN,
        ...
        settings_button = buttons.Button(settings.MARGIN + 2 * settings.BT_DIST,
                                         ...
                                         "Opções", "blue_button")
        quit_button = buttons.Button(settings.MARGIN + 3 * settings.BT_DIST, settings.SCREEN_HEIGHT - settings.MARGIN,
                                     "Sair", "blue_button")
...
def pop_up(player, enemy, screen, qa, itens_icon):
    ...
    while time_lim >= 0:
        ...
        atck2 = buttons.ButtonFight(position_x, position_y, ans[sample[0]], "withe_button", sample[0])

        space = TILESIZE
        atck1 = buttons.ButtonFight(position_x - atck2.rectangle.w - space, position_y,
                                    ans[sample[1]], "withe_button", sample[1])

        atck3 = buttons.ButtonFight(position_x + atck2.rectangle.w + space, position_y,
        ...
        q_surface = pygame.Surface((TILESIZE * 10, TILESIZE * 4))  # the size of your rect
        q_surface.set_alpha(128)  # alpha level
        q_surface.fill(WHITE)  # this fills the entire surface
        rect = q_surface.get_rect(midtop=(tempo_title.midbottom[0], tempo_title.midbottom[1] + TILESIZE / 4))

        question_text = font_text.render(question, True, WHITE)
        question_rect = question_text.get_rect(center=rect.center)

        screen.blit(q_surface, rect)
